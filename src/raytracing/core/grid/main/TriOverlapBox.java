...
public class TriOverlapBox {
    ...
    public static boolean tri_overlap_box(Point3f v0, Vector3f e1, Vector3f e2, Vector3f n, Point3f min, Point3f max) {
        if (!plane_overlap_box(n, Vector3f.dot(v0.asVector3f(), n), min, max))
            return false;
        
        Vector3f v1 = v0.sub(e1.asPoint3f());
        Vector3f v2 = v0.sub(e2.asPoint3f());
        
        boolean bounds_check = true, cross_checks = true;
        ...
        if (cross_checks) {
            Point3f center    = (max.add(min.asVector3f())).mul(0.5f);
