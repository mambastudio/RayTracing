...
import coordinate.memory.nativememory.algorithm.SerialNativeObjectAlgorithm;
...
import raytracing.accel.grid.onheap.base.Range;
...
import raytracing.accel.grid.offheap.base.NLevel;
 ...
 * @author user
 */
public class NBuild extends NGridAbstract{
    NHagrid hagrid;
    
    public NBuild(NHagrid hagrid)
    {
        this.hagrid = hagrid;
    }
    ...
    public void mark_kept_refs(
        ...
        {            
            ...
            int value = ((cell_id >= 0) && (entries.get(cell_id, new NEntry()).log_dim == 0)) ? 1 : 0;              
    ...
    public void compute_split_masks(NativeInteger cell_ids,
        ...
        {            
            ...
            NCell cell = cells.get(cell_id).copy();          
    ...
    public void mark_kept_cells(
        ...
        {
            if (id >= num_cells) return;
            kept_cells.set(id, (entries.get(id, new NEntry()).log_dim == 0) ? 1 : 0);
        }
    }
    ...
    public void compute_cell_ranges(NativeInteger cell_ids, NativeObject<NCell> cells, int num_refs) {
        ...
        for(int id = 0; id<num_refs; id++)
        {
            ...
            if (cell_id != next_id) {
                NCell cell_1 = cells.get(cell_id);
                NCell cell_2 = cells.get(next_id);
                cell_1.end = id + 1;
                cell_2.begin = id + 1;
    ...
    public void emit_top_cells(NativeObject<NCell> new_cells, int num_cells) {
        for(int id = 0; id<num_cells; id++)
        {            
            if (id >= num_cells) return;
    ...
    {        
        ...
        {            
            ...
            NCell cell = cells.get(cell_ids.get(id));
    ...
    public void split_refs(
        ...
        for(int id = 0; id<num_split; id++)
        {
            ...
            int ref = ref_ids.get(id); 
            int begin = entries.get(cell_id, new NEntry()).begin;
    ...
    public void first_build_iter(
            ...
            int[] grid_shift, ArrayList<NLevel> levels) 
    {
        ...
        NativeObject<NCell> new_cells   = new NativeObject(NCell.class, num_top_cells + 0);
                ...
                int iter)
    {
        ...
        NativeObject<NCell> cells           = levels.get(levels.size()-1).cells;
        ...
        int num_new_cells = par.exclusive_scan(par.transform(entries, e->new NEntry(e).log_dim == 0 ? 0 : 8), 
    ...
    public void concat_levels(ArrayList<NLevel> levels, NGrid grid) {
        SerialNativeIntegerAlgorithm par = new SerialNativeIntegerAlgorithm();        
        ...
        par.sort_pairs(cell_ids, ref_ids, new_cell_ids, new_ref_ids, total_refs);
    ...
    {        
        ...
        ArrayList<NLevel> levels = new ArrayList();
