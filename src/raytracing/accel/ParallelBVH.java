...
import thread.model.ThreadPool;
 ...
 * @author user
 */
public class ParallelBVH implements AbstractAccelerator<Ray, Intersection, Primitive, BoundingBox>{
    ...
    public void build(Primitive primitives) {
        ...
        //Parallel build
        threadPool.execute(new KernelBuild(root, 0, objects.length));
        threadPool.waitAndShutdown();     
        
        long time2 = System.nanoTime();
        System.out.println(time2 - time1);
    }
    
    private synchronized void setNode(BVHNode parent, BVHNode left, BVHNode right)
    {     
    ...
    public class BVHNode
    {
        BoundingBox bounds;
        boolean isLeaf;
        int left, right, start, end, axis;   
    } 
    
    public class KernelBuild implements Runnable
    {
        BVHNode parent;
        int start, end;
        
        public KernelBuild(BVHNode parent, int start, int end)
        {
            this.parent = parent;
            this.start = start;
            this.end = end;
        ...
        {                  
            ...
            //Subdivide
            threadPool.execute(new KernelBuild(left, start, mid));
            threadPool.execute(new KernelBuild(right, mid, end));
        }
    }
}
